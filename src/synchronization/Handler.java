...
public class Handler implements GameInterface
{
    ...
    // Collision handler
    public void coll(GameObject player, GameObject food){
        if(player.getX() < food.getX()+player.getWidth() && player.getX() > food.getX()-player.getWidth()){
            if(player.getY() < food.getY()+player.getHeight() && player.getY() > food.getY()-player.getHeight()){
                food.setX(rand.nextInt(640));
                food.setY(rand.nextInt(480));
            }
        }
    }
